...
func SaveNodeStats(stats *NodeStats) error {
	...
	query := `UPDATE node_stats SET
		latency = ?, available = ?, last_check = ?
		WHERE node_id = ?`

	result, err := db.Exec(query,
		stats.Latency, stats.Available, stats.LastCheck,
		stats.NodeID,
	)
	if err != nil {
	...
	if rows == 0 {
		query = `INSERT INTO node_stats
			(node_id, latency, available, last_check)
			VALUES (?, ?, ?, ?)`

		result, err = db.Exec(query,
			stats.NodeID, stats.Latency, stats.Available, stats.LastCheck,
		)
		if err != nil {
...
func GetNodeStats(nodeID int) (*NodeStats, error) {
	query := `SELECT id, node_id, latency, tx_bytes, rx_bytes, last_check, available
		FROM node_stats WHERE node_id = ?`

	stats := &NodeStats{}
	err := db.QueryRow(query, nodeID).Scan(
		&stats.ID, &stats.NodeID, &stats.Latency,
